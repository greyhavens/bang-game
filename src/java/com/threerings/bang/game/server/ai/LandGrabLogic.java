    ...
    implements PieceCodes
{
        ...
        Piece[] pieces, Unit unit, PointSet moves, PointSet attacks)
    {
        ...
        for (Homestead stead : _steads) {
            if (stead.owner == unit.owner || (!isBShot && stead.owner < 0)) {
                continue;
            }
            int value = Math.max(20 - unit.getDistance(stead), 0) * 10 +
                        stead.damage + Math.max(20 - stead.getDistance(
                                    control.x, control.y), 0) * 5;
            if (isBShot) {
        ...
        if (ostead != null) {
            ...
            if (mv != null) {
                if (isBShot && ostead.owner < 0 && 
                    ...
                    executeOrder(unit, mv.x, mv.y, getBestTarget(
                                pieces, unit, mv.x, mv.y, TARGET_EVALUATOR));
                    return;
                }
        ...
        Piece target = getBestTarget(
                        pieces, unit, unit.x, unit.y, TARGET_EVALUATOR);
        if (target != null) {
