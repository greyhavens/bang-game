    ...
    implements PieceCodes
{
    ...
    /** Handles a request to toggle props. Generated by
     * the {@link #TOGGLE_PROPS} command. */
    public void handleToggleProps (Object source)
    {
        String[] scids = ScenarioFactory.getScenarios(
                BangCodes.TOWN_IDS[BangCodes.TOWN_IDS.length-1]);
        ArrayList<String> validSc = new ArrayList<String>();
        for (int ii = 0; ii < scids.length; ii++) {
            if (_panel.propChecks[ii].getState()) {
                validSc.add(scids[ii]);
            }
        }
        int size = validSc.size();
        ...
        for (Piece p : pieces) {
            ...
            Prop prop = (Prop)p;
            boolean valid = false;
            for (int ii = 0; ii < size && !valid; ii++) {
                valid = prop.isValidScenario(validSc.get(ii));
            }
