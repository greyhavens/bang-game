        ...
        throws InvocationException
    {
        ...
        case BangMarshaller.GET_BOARD:
            ((BangProvider)provider).getBoard(
                source, (BangService.BoardListener)args[0]
            );
            return;

        case BangMarshaller.ORDER:
            ((BangProvider)provider).order(
                source, ((Integer)args[0]).intValue(), ((Short)args[1]).shortValue(), ((Short)args[2]).shortValue(), ((Integer)args[3]).intValue(), (InvocationService.ResultListener)args[4]
            );
            return;

        case BangMarshaller.PLAY_CARD:
            ((BangProvider)provider).playCard(
                source, ((Integer)args[0]).intValue(), args[1], (InvocationService.ConfirmListener)args[2]
            );
            return;

        case BangMarshaller.REPORT_PERFORMANCE:
            ((BangProvider)provider).reportPerformance(
                source, (String)args[0], (String)args[1], (int[])args[2]
            );
            return;

        case BangMarshaller.SELECT_TEAM:
            ((BangProvider)provider).selectTeam(
                source, ((Integer)args[0]).intValue(), (String[])args[1], (int[])args[2]
            );
            return;
