        ...
        throws InvocationException
    {
        ...
        case GangPeerMarshaller.ADD_TO_COFFERS:
            ((GangPeerProvider)provider).addToCoffers(
                source, (Handle)args[0], (String)args[1], ((Integer)args[2]).intValue(), ((Integer)args[3]).intValue(), (InvocationService.ConfirmListener)args[4]
            );
            return;
        ...
        case GangPeerMarshaller.BUY_GANG_GOOD:
            ((GangPeerProvider)provider).buyGangGood(
                source, (Handle)args[0], (String)args[1], (Object[])args[2], ((Boolean)args[3]).booleanValue(), (InvocationService.ConfirmListener)args[4]
            );
            return;

        case GangPeerMarshaller.CHANGE_MEMBER_RANK:
            ((GangPeerProvider)provider).changeMemberRank(
                source, (Handle)args[0], (Handle)args[1], ((Byte)args[2]).byteValue(), (InvocationService.ConfirmListener)args[3]
            );
            return;

        case GangPeerMarshaller.CHANGE_MEMBER_TITLE:
            ((GangPeerProvider)provider).changeMemberTitle(
                source, (Handle)args[0], (Handle)args[1], ((Integer)args[2]).intValue(), (InvocationService.ConfirmListener)args[3]
            );
            return;

        case GangPeerMarshaller.GET_UPGRADE_QUOTE:
            ((GangPeerProvider)provider).getUpgradeQuote(
                source, (Handle)args[0], (GangGood)args[1], (InvocationService.ResultListener)args[2]
            );
            return;

        case GangPeerMarshaller.GRANT_ACES:
            ((GangPeerProvider)provider).grantAces(
                source, (Handle)args[0], ((Integer)args[1]).intValue()
            );
            return;
        ...
        case GangPeerMarshaller.HANDLE_INVITE_RESPONSE:
            ((GangPeerProvider)provider).handleInviteResponse(
                source, (Handle)args[0], ((Integer)args[1]).intValue(), (Handle)args[2], ((Boolean)args[3]).booleanValue(), (InvocationService.ConfirmListener)args[4]
            );
            return;

        case GangPeerMarshaller.INVITE_MEMBER:
            ((GangPeerProvider)provider).inviteMember(
                source, (Handle)args[0], (Handle)args[1], (String)args[2], (InvocationService.ConfirmListener)args[3]
            );
            return;

        case GangPeerMarshaller.MEMBER_ENTERED_HIDEOUT:
            ((GangPeerProvider)provider).memberEnteredHideout(
                source, (Handle)args[0], (AvatarInfo)args[1]
            );
            return;

        case GangPeerMarshaller.MEMBER_LEFT_HIDEOUT:
            ((GangPeerProvider)provider).memberLeftHideout(
                source, (Handle)args[0]
            );
            return;

        case GangPeerMarshaller.PROCESS_OUTFITS:
            ((GangPeerProvider)provider).processOutfits(
                source, (Handle)args[0], (OutfitArticle[])args[1], ((Boolean)args[2]).booleanValue(), ((Boolean)args[3]).booleanValue(), (InvocationService.ResultListener)args[4]
            );
            return;

        case GangPeerMarshaller.REMOVE_FROM_GANG:
            ((GangPeerProvider)provider).removeFromGang(
                source, (Handle)args[0], (Handle)args[1], (InvocationService.ConfirmListener)args[2]
            );
            return;
        ...
        case GangPeerMarshaller.SEND_SPEAK:
            ((GangPeerProvider)provider).sendSpeak(
                source, (Handle)args[0], (String)args[1], ((Byte)args[2]).byteValue()
            );
            return;

        case GangPeerMarshaller.SET_AVATAR:
            ((GangPeerProvider)provider).setAvatar(
                source, ((Integer)args[0]).intValue(), (AvatarInfo)args[1]
            );
            return;

        case GangPeerMarshaller.SET_BUCKLE:
            ((GangPeerProvider)provider).setBuckle(
                source, (Handle)args[0], (BucklePart[])args[1], (InvocationService.ConfirmListener)args[2]
            );
            return;

        case GangPeerMarshaller.SET_STATEMENT:
            ((GangPeerProvider)provider).setStatement(
                source, (Handle)args[0], (String)args[1], (String)args[2], (InvocationService.ConfirmListener)args[3]
            );
            return;

        case GangPeerMarshaller.TRADE_COMPLETED:
            ((GangPeerProvider)provider).tradeCompleted(
                source, ((Integer)args[0]).intValue(), ((Integer)args[1]).intValue(), (String)args[2]
            );
            return;
